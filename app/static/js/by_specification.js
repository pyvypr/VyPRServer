...
var subatom_click = function(obj){
	...
	make_lines_clickable(lines_list, subtree, $(obj).parent(), sub_index);
}

var make_lines_clickable = function(lines_list, subtree, obj, sub_index){
	...
	$.get("/get_atom_type/"+atom_index+"/"+inst_point_id+"/", function(atom_type) {
		...
		if (atom_type == "simple"){
			dropdown_content = '<p class="dropdown-menu-option"> Plot observed values from this point </p>';
		}
		else if (atom_type == "timeBetween") {
			dropdown_content = '<p class="dropdown-menu-option"> Fix this point and select the other one</p>';
		}
		...
		for (var i=0; i<lines_list.length; i++){
			...
			$("#line-number-"+no).append('<div class="dropdown-content" id="'+no+'">'+dropdown_content+'</div>');
		}

		menu_options = $(".dropdown-menu-option");
		for (var i=0; i<menu_options.length; i++){
			var option = menu_options[i];
			if (atom_type == "simple"){
				var id;
				var inst_point_id;
				var line_no = $(option).parent().attr('id');
				for (subatom_index in subtree){
					if (subatom_index == sub_index){
						var list = subtree[subatom_index];
						for (var j=0; j<list.length; j++){
							if (list[j]["code_line"]==line_no){
								id = list[j]["id"];
							}
						}
					}
				}
				option.onclick = function(){plot_function([id]);};
			}
			else {
				option.onclick = function(){select_another_point(event, subtree, sub_index, $(this).parent().attr('id'));};
			}
		}
...
var plot_function = function(inst_points_list){
	console.log(inst_points_list.length)
	if (inst_points_list.length == 1){
		//atom is simple
		console.log(inst_points_list[0])
	}
	else{
		//atom is mixed or timeBetween
	}
}

var select_another_point = function(evt, subtree, sub_index, line_no) {
	console.log(subtree);
	var id;
	var inst_point_id;
	for (subatom_index in subtree){
		if (subatom_index != sub_index){
			var inst_points_list = subtree[subatom_index];
		}
		else{
			var list = subtree[subatom_index];
			for (var i=0; i<list.length; i++){
				if (list[i]["code_line"]==line_no){
					id = list[i]["id"];
				}
			}
		}
	...
	for (var i=0; i<inst_points_list.length; i++){
		var no = inst_points_list[i]["code_line"];
		var new_inst_point_id = inst_points_list[i]["id"];
		var color = $("#line-number-"+no).attr('save-background-color');
		$("#line-number-"+no).attr('style',"background-color : " + color);
		$("#line-number-"+no).attr('onclick', "plot_function(["+id+","+new_inst_point_id+"])");
	}
}
